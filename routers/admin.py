...
from app.service.web3_service import web3_ganache
from app.schemas.transfer_request import TransferRequest
from app.service.web3_service import send_eth, get_account_balance
...
async def approve_loan(loan_id: int, user: user_dependency, db: db_dependency, approve: bool):
    ...
    # Fetch the borrower's account (the one who asked the loan)
    ...
    # Fetch the admin's account (loan provider) -> in future be any user
    admin_account = db.query(Account).filter(Account.user_id == user.get("id")).first()
    if not admin_account:
        raise HTTPException(status_code=404, detail="Admin's account not found")

    # Fetch the borrower's User (the one who asked the loan)
    ...
    if approve:
        # Ensure admin (loaner) has enough balance to transfer the loan amount
        ...
        # ✅ Transfer ETH from admin to borrower using web3_service
        try:
            tx_hash = send_eth(
                from_address=user.get("public_key"),  # admin sends
                to_address=borrower_profile.public_key,  # borrower receives
                amount=loan.amount,
            )
        except Exception as e:
            raise HTTPException(
                status_code=500,
                detail=f"Loan transfer failed on the blockchain: {e}"
            )
        ...
        loan.end_date = (datetime.now() + timedelta(minutes=loan.duration_months.value)).strftime("%Y-%m-%d %H:%M:%S")

        # Update start date
        loan.start_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # update accounts on db
